...
ylabel(tl, 'Average cluster size', 'Fontweight', 'bold', 'FontSize', 20);
for i = 1:numel(v)
    nt = nexttile;
    errorbar(Nl(Nl(:, 3) == v(i), 2), Nl(Nl(:, 3) == v(i), pd(i)+3), Nle(Nle(:, 3) == v(i), pd(i)+3), 'o', 'Color', [CO6(mod(v(i), 10), :) 1], 'LineWidth', 2, 'MarkerSize', 0.1); hold on;
    mdl = fitlm(Nl(Nl(:, 3) == v(i), 2), Nl(Nl(:, 3) == v(i), pd(i)+3), 'Weights', Nle(Nle(:, 3) == v(i), pd(i)+3));
    [Ypred, Yci] = predict(mdl,Nl(Nl(:, 3) == v(i), 2));
    plot(Nl(Nl(:, 3) == v(i), 2), Ypred, '-', 'Color', [CO6(mod(v(i), 10), :) 1], 'LineWidth', 2);
    fill([Nl(Nl(:, 3) == v(i), 2)', fliplr(Nl(Nl(:, 3) == v(i), 2)')], [Yci(:, 2)' fliplr(Yci(:, 1)')], CO6(mod(v(i), 10), :), 'EdgeColor', 'none', 'FaceAlpha', 0.4);
    xlabel('');
    ylabel('');
    if mod(v(i), 10) > 3
        set(gca, 'Box', 'on', 'FontSize', 20, 'LineWidth', 1, 'XTick', 15:5:30, 'YTickLabel', '');
    else
        set(gca, 'Box', 'on', 'FontSize', 20, 'LineWidth', 1, 'XTick', 15:5:30);
    end
    axis([14 33 0 69.99]);
    CI = coefCI(mdl);
