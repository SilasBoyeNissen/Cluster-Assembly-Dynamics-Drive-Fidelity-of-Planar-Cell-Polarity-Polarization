...
for v = [11 12 14 13 15 16]
    nexttile;
    errorbar(Nl(Nl(:, 3) == v, 2), Nl(Nl(:, 3) == v, pd+3), Nle(Nle(:, 3) == v, pd+3), 'o', 'Color', CO6(mod(v, 10), :), 'LineWidth', 2, 'MarkerSize', 0.1); hold on;
    mdl = fitlm(Nl(Nl(:, 3) == v, 2), Nl(Nl(:, 3) == v, pd+3), 'Weights', Nle(Nle(:, 3) == v, pd+3));
    [Ypred, Yci] = predict(mdl,Nl(Nl(:, 3) == v, 2));
    plot(Nl(Nl(:, 3) == v, 2), Ypred, '-', 'Color', [CO6(mod(v, 10), :) 0.8], 'LineWidth', 2);
    ...
    text(15, 13.5, ['{\bf Mean = ' num2str(round(Ypred)) '}, 95%CI [' num2str(floor(Yci(1))) ', ' num2str(ceil(Yci(2))) ']' newline ...
        ...
        '{\bf P-value = ' num2str(round(table2array(mdl.Coefficients(2, 4)), 1, 'significant')) '}, n=' num2str(mdl.NumObservations) ', N=' num2str(sum(N(N(:, 3) == v, pd+3))) ], 'FontSize', 14);
