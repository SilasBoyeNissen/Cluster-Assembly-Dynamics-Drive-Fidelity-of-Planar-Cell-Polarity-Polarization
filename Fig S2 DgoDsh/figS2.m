...
% Figure S2: Dgo and Dsh
pd = 4;
figure(1); clf;
set(gcf, 'Color', 'w', 'Position', [0 550 900 450]);
tl = tiledlayout(1, 2, 'TileSpacing', 'none', 'Padding', 'tight');
xlabel(tl, 'Number of molecules in clusters', 'Fontweight', 'bold', 'FontSize', 16);
ylabel(tl, '1 - Cumulative Distribution Function', 'Fontweight', 'bold', 'FontSize', 16);
for v = 11:12
    TXT = {};
    nt = nexttile;
    for apf = 1:2
    ...
    ylabel('');
    if mod(v, 10) > 1
        set(gca, 'Box', 'on', 'FontSize', 16, 'LineWidth', 1, 'XTick', 0:20:40, 'YScale', 'log', 'YTickLabel', '');
    else
        set(gca, 'Box', 'on', 'FontSize', 16, 'LineWidth', 1, 'XTick', 0:20:40, 'YScale', 'log');
    end
...
print('figS2a', '-dpdf', '-r300');

figure(2); clf;
set(gcf, 'Color', 'w', 'Position', [0 0 900 450]);
tl = tiledlayout(1, 2, 'TileSpacing', 'none', 'Padding', 'tight');
xlabel(tl, 'Pupal age [hours APF]', 'Fontweight', 'bold', 'FontSize', 16);
ylabel(tl, 'Average cluster size', 'Fontweight', 'bold', 'FontSize', 16);
for v = 11:12
    nt = nexttile;
    errorbar(Nl(Nl(:, 3) == v, 2), Nl(Nl(:, 3) == v, pd+3), Nle(Nle(:, 3) == v, pd+3), 'o', 'Color', [CO6(mod(v, 10), :) 1], 'LineWidth', 2, 'MarkerSize', 0.1); hold on;
    ...
    ylabel('');
    if mod(v, 10) > 1
        set(gca, 'Box', 'on', 'FontSize', 14, 'LineWidth', 1, 'XTick', 15:5:30, 'YTickLabel', '');
    else
        set(gca, 'Box', 'on', 'FontSize', 14, 'LineWidth', 1, 'XTick', 15:5:30);
    end
    axis([14 33 0 69.99]);
    CI = coefCI(mdl);
