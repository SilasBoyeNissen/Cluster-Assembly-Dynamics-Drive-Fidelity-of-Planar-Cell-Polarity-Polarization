...
pd = 4;
figure(1); clf;
set(gcf, 'Color', 'w', 'Position', [10 10 800 460]);
tl = tiledlayout(1, 2, 'TileSpacing', 'none', 'Padding', 'tight');
xlabel(tl, 'Pupal age [hours APF]', 'Fontweight', 'bold', 'FontSize', 17);
ylabel(tl, 'Average cluster size', 'Fontweight', 'bold', 'FontSize', 17);
k = 0;
vv = [46 47];
for v = vv
    k = k + 1;
    ...
    ylabel('');
    if v ~= 46
        set(gca, 'Box', 'on', 'FontSize', 17, 'LineWidth', 1, 'XTick', XT, 'YTickLabel', '');
    else
        set(gca, 'Box', 'on', 'FontSize', 17, 'LineWidth', 1, 'XTick', XT, 'YTick', '');
    end
...
set(gca, 'XTick', XT);
set(nexttile(1), 'YTick', 0:5:30, 'XTick', XT);
f = gcf; f.PaperSize = [f.PaperPosition(3) f.PaperPosition(4)];
print('figS9', '-dpdf', '-r300');