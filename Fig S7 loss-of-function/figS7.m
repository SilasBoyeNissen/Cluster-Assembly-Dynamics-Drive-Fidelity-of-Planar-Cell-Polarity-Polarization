...
for v = [13 33 15 35 16 36 14 38 14 34]
    ...
    errorbar(Nl(Nl(:, 3) == v, 2), Nl(Nl(:, 3) == v, pd+3), Nle(Nle(:, 3) == v, pd+3), 'o', 'Color', cc, 'LineWidth', 2, 'MarkerSize', 0.1); hold on;
    mdl = fitlm(Nl(Nl(:, 3) == v, 2), Nl(Nl(:, 3) == v, pd+3), 'Weights', Nle(Nle(:, 3) == v, pd+3));
    [Ypred, Yci] = predict(mdl,Nl(Nl(:, 3) == v, 2));
    plot(Nl(Nl(:, 3) == v, 2), Ypred, '-', 'Color', cc, 'LineWidth', 2);
    fill([Nl(Nl(:, 3) == v, 2)', fliplr(Nl(Nl(:, 3) == v, 2)')], [Yci(:, 2)' fliplr(Yci(:, 1)')], cc, 'EdgeColor', 'none', 'FaceAlpha', 0.4);
    xlabel('');
    ylabel('');
    if mod(v, 10) == 3
        set(gca, 'Box', 'on', 'FontSize', 20, 'LineWidth', 1, 'XTick', XT);
    else
        set(gca, 'Box', 'on', 'FontSize', 20, 'LineWidth', 1, 'XTick', XT, 'YTickLabel', '');
    end
    axis([14 33 0 39.99]);
    CI = coefCI(mdl);
...
set(gca, 'XTick', XT);
set(nexttile(6), 'YTick', 0:10:60);
set(nexttile(1), 'YTick', 0:10:60, 'XTick', XT);
